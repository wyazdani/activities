...
const mongoClientAllowedOptions = ['useNewUrlParser', 'useUnifiedTopology'];
...
class MongoClients {
  ...
  static async getClient(logger, configKey) {
    ...
    const allowedDbOptions = _.pick(config.get(`${configKey}.options`), mongoClientAllowedOptions);
    const options = {maxPoolSize: (configKeys[configKey] > 5) ? configKeys[configKey] : 5, ...allowedDbOptions};
